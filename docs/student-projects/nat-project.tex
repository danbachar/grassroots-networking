...
witness source appearing in that history is a logged discard condition, and every dial-back in the
filtering arm must originate from an endpoint never contacted. Running the same test with and without
...
perform. Second, an off-path dial must target the requester's observed source address or carry a
proof-of-cost comparable to autoNAT v2's, whose cost in bytes and latency we report, as it is both a
metered-data consideration and a latency confound. Third, we rate-limit per requester and refuse to
