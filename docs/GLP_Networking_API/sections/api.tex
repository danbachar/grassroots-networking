...
\rowcolor{rowgray}
Connectivity status & \func{onConnectivityStatus(cb)} & Callback when the networking layer detects a change in the agent's public IP address.  Triggers the rendezvous protocol (Section~\ref{sec:rendezvous}).  Fired on startup and on address change. \dan{The Dart implementation names this \func{onConnectivityStatusChanged} --- the spec name reads like a status getter, but this is a change-event callback. It receives \code{(oldAddress, newAddress)} as \code{ip:port} strings, either of which may be \code{null} when no public address is available; the kind of change is derived from null comparisons (startup or gain: \code{old=null, new!=null}; loss: \code{old!=null, new=null}; update: both non-null and different). Consider adopting this name and signature in the spec.} \\
...
\rowcolor{rowgray}
Receive payload & \func{onReceive(cb)} & Register callback for incoming payloads. The callback receives the sender's public key and the byte payload. \dan{The Dart implementation additionally exposes a richer \func{onMessageReceived} callback with two extra arguments: \code{messageId} (a transport-assigned id, useful for dedup and ACK correlation; also dispatched via the Redux \code{MessageReceivedAction}) and \code{transport} (the peer's currently preferred transport at delivery time, for logging only --- not authoritative for the arrival transport). Both \func{onReceive} and \func{onMessageReceived} fire for every accepted message.} \\
\bottomrule
\end{longtable}
%------------------------------------------------------------------------------
